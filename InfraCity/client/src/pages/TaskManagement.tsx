...
export function TaskManagement() {
  ...
  const handleOptimizeRoute = async () => {
    ...
    try {
      ...
      const response = await getOptimizedRoute(selectedTasks)
                  ...
                  {statusTasks.map((task) => (
                    <Card
                      ...
                      className={`bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl border-slate-200/50 dark:border-slate-700/50 hover:shadow-lg transition-all duration-200 cursor-pointer ${
                        selectedTasks.includes(task._id) ? 'ring-2 ring-blue-500' : ''
                      }`}
                      onClick={() => {
            ...
            <CardContent>
              <div className="space-y-4">
                {filteredTasks.map((task) => (
                  <div key={task._id} className="flex items-center justify-between p-4 border rounded-lg hover:bg-slate-50 dark:hover:bg-slate-800/50">
                    <div className="flex items-center space-x-4">
                      <input
                        type="checkbox"
